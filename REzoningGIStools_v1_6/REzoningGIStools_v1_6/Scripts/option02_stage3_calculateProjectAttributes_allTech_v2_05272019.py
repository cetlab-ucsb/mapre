...
effLoss = 1 - arcpy.GetParameter(15) ## Assume wind losses (15%) without outage rate (2%). So default value should be 0.85;
outageRate = 1 - arcpy.GetParameter(16) ## solar PV only # RD: for both wind and solar. Give option to user to set this to zero for the IRENA dataset.
...
def CSPcfCalc_6h(DNI, units): ## for 6h storage
    ...
    CF_6h = (33.431*math.log(DNI_kWhm2y_6h) - 212.57)/100 # to make it a fraction # RD; Does this include outage rate? losses?
    return CF_6h
'''
#####################################################################################
#### --------------------------------GEOPROCESSES--------------------------------####
...
crf_mod_cf_degrad = (dr_mod_cf_degrad * (1 + dr_mod_cf_degrad)**plantLifetime) / (((1+dr_mod_cf_degrad)**plantLifetime) - 1)
...
for row in cursor:        
    ...
    roadLCOEnew = (roadCost * roadDist) / 50 / electGenPkW_npv # RD 05272019: Verify if roadLCOEnew is same as roadLCOE with 0 degradation. If same, change roadLCOEnew to roadLCOE.
    ...
    if inputDataPath["d_trans"][0] == "yes":
            ...
            transLCOEnew = (transCost*transDist + subCost) / electGenPkW_npv # RD 05272019: Verify if transLCOEnew is same as transLCOE with 0 degradation. If same, change transLCOEnew to transLCOE.
    ...
    if inputDataPath["d_sub"][0] == "yes":
            ...
            subLCOEnew = (transCost*subDist + subCost) / electGenPkW_npv # RD 05272019: Verify if subLCOEnew is same as subLCOE with 0 degradation. If same, change subLCOEnew to subLCOE.
