...
class Attributes:
    def __init__(self, technology, projectsIn, projectsOut, resourceInput, csvInput, templateRaster,
        ...
        ## SPATIAL INPUTS
        self.technology = str(technology)  ##"solar", "wind"
        ...
        self.effLoss = effLoss  ## Assume wind losses (15%) without outage rate (2%). So default value should be 0.85; ## 0.17
        self.outageRate = outageRate  ## 0.02 ## solar PV only # RD: for both wind and solar. Give option to user to set this to zero for the IRENA dataset.
    ...
    def addAttributes(self):
        ...
        losses = 1-((1-self.outageRate) * (1-self.effLoss) ) # RD 052019: Add outage rates. User can choose to set this as 0
        ...
        if arcpy.GetCount_management(projects)[0] != "0":
            ...
            for dataCategory in inputDataPath:
                if inputDataPath[dataCategory][0] == "yes" and inputDataPath[dataCategory][1] == "mean":
                    ...
                    if mean_fieldName == "m_rangeland":
                        arcpy.CalculateField_management(projects, mean_fieldName, """!m_rangeland!/200""")
                    if mean_fieldName == "m_slope":
                        arcpy.CalculateField_management(projects, mean_fieldName, """!m_slope!/100""")


            for dataCategory in inputDataPath:
                if inputDataPath[dataCategory][0] == "yes" and inputDataPath[dataCategory][1] == "majority":
                    print("majority")
                    majority_feature = inputDataPath[dataCategory][2]
                    majority_feature_int = Int(majority_feature)
                    majority_fieldName = dataCategory

                    ## Calculate average values
                    arcpy.sa.ZonalStatisticsAsTable(projects_raster, "Value", \
                                                    majority_feature_int, "in_memory/zonalStats1", "DATA", "MAJORITY")
                    # Join zonal statistics table of "MAJORITY" values to target project polygon, projects
                    arcpy.JoinField_management(projects, idField, \
                                               "in_memory/zonalStats1", "Value", "MAJORITY")
                    arcpy.AddField_management(projects, majority_fieldName, "DOUBLE")
                    arcpy.CalculateField_management(projects, majority_fieldName, "!MAJORITY!", "PYTHON_9.3")
                    arcpy.DeleteField_management(projects, "MAJORITY")
                    arcpy.AddMessage("Zonal stats for " + majority_fieldName + " is complete")
            ...
            for dataCategory in inputDataPath:
                if inputDataPath[dataCategory][0] == "yes" and inputDataPath[dataCategory][1] == "coverage":
                    ...
                    mean_feature = Con(IsNull(mean_feature), 0, 1)
            ...
            if self.RQtype == "kWh/m2-day" and (self.technology == "solar" or self.technology == "CSP"):
            ...
            print("Input file empty, therefore outputting empty result file")
