...
for row in cursor:        
    ...
    for turbineArea in turbineClasses:
        ...
        if proportion == 0:
            ...
            LCOEgen = (turbineClasses[turbineArea][0]*CRF + fixedGenOMcost)/(hours*turbineClasses[turbineArea][1]*windLosses) + variableGenOMcost
    ...
    for totalLCOEfield in LCOEtotTrans_List:   
        transLCOE = (transCost*transDist + subCost)*CRF/(LCOEtotTrans_List[totalLCOEfield][0]*hours*windLosses)
    ...
    row.setValue("l_tra_ch", round((transCost*transDist + subCost)*CRF/(CF_chosen*hours*windLosses),2)) ## calculate the "TransCost" using the chosen turbine"
    ...
    row.setValue("l_sub_ch", round((transCost*subDist + subCost)*CRF/(CF_chosen*hours*windLosses),2)) ## calculate the "SubCost" using the chosen turbine"        
