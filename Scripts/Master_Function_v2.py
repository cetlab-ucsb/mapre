...
ap.add_argument("-ed", "--ed_files", required=False, nargs='*',
                help="list of file names (shp or raster) (without extension) requiring euclidean distance rasters separated by spaces",
                type=str)
ap.add_argument("-md", "--max_dist", required=False, nargs='+', help="max distance for eucl distance (optional)",
                type=str)
ap.add_argument("-lf", "--lulc_file", required=False, nargs='+', help="name of land use land cover file to extract",
                type=str)
...
ap.add_argument("-wn", "--wind_nonag", required=False, nargs='+',
                help="which land use categories to extract (include) for non-ag wind (numbers separated by commas)",
                type=str)
ap.add_argument("-wa", "--wind_ag", required=False, nargs='+',
                help="which land use categories to extract (include) for ag wind (numbers separated by commas)",
                type=str)
ap.add_argument("-cf", "--country_file", required=True, nargs='+',
                help="name of projected world country boundaries file to extract (projected.gdb\country_file_Projected)",
                type=str)
ap.add_argument("-c", "--country_list", required=True, nargs='*',
                help="list of countries in quotes and separated by spaces", type=str)
...
for i in range(len(csv_file)):
    ...
    processBool = csv_file[6][i]
    templateBool = csv_file[7][i]
    eucBool = csv_file[8][i]
    boundsBool = csv_file[9][i]
    lulcBool = csv_file[9][i]
