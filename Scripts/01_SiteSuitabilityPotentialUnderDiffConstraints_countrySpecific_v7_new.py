...
dateAnalysis = "01032015" ##^^ this date will be used to create the output database
...
if not(os.path.exists(outputFolder + gdbName)): # Create new fgdb if one does not already exist
    ...
    arcpy.CreateFileGDB_management(outputFolder, gdbNameForCreatingFGDB)
...
templateRaster = defaultInputWorkspace + "technoeconomic.gdb\\mergedDEM_GADM_500" ##^^ enter path to DEM data
...
processLulc = "No" ##^^ "Yes" or "No" depending on whether you want to create new lulc euclidean distance exclusions
...
lulc = defaultInputWorkspace + "Environmental.gdb\\LULC_Projected2" ##^^ put your lulc data here, if you're using country-specific lulc, if not, use default
...
import csv
csvInput2 = yourSpace + "INPUTS\\Countries\\demand2030.csv"
...
import csv
csvInput = yourSpace + "INPUTS\\Countries\\" + country + "\\" + country + ".csv"
...
for tech in technologyList:
    ...
    if technology == "wind":
        ...
        if thresholdTest =="No":
            whereResource = ["Value >= 300"] ## enter resource threshold [w/m2 or m/sec]
        resource = defaultInputWorkspace + "Resources.gdb\\wind_powerdensity_100m_Pr" ##^^ enter local path to resource file (either wind speed or wind power density)
    ...
    if technology == "solarPV":
        ...
        if thresholdTest =="No":
            whereResource = ["Value >= 1"]  ## enter resource threshold [w/m2 or m/sec]
        resource = defaultInputWorkspace + "Resources.gdb\\GHI_Projected" ##^^ enter local path to resource file
    ...
    if technology == "solarCSP":
        ...
        resource = defaultInputWorkspace + "Resources.gdb\\DNI_Projected" ##^^ enter local path to resource file
        whereSlope = "Value <= 5" # slope threshold in percent
    ...
    ##                    *roadsCon  ## Road areas are not excluded
        ...
        if technology == "solarCSP":
            inRaster = defaultInputWorkspace + "Resources.gdb\\DNI_Projected"
            thresholdSc1 = 280
            thresholdList = [260, 270, 280, 290, 300]
        if technology == "solarPV":
            inRaster = defaultInputWorkspace + "Resources.gdb\\GHI_Projected"
            thresholdSc1 = 250
            thresholdList = [230, 240, 250, 260, 270]
        if technology == "wind":
            inRaster = defaultInputWorkspace + "Resources.gdb\\wind_powerdensity_100m_Pr"
            thresholdSc1 = 300
